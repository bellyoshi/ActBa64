...
prototype3/
│
├── *.abp / *.pj / *.idx     # ツールソース（abpc / abc / abassembler / ablinker）
├── Include/                 # 標準ヘッダ
├── test/                    # 回帰テスト
│
├── bin/                     # 出力先のルート
...
`.pj` の `#OUTPUT_RELEASE` は **`bin\stage0\`**（例: `.\bin\stage0\abpc.exe`）。  
...
3. IDE からビルドする（`abpc` / `abc` / `abassembler` / `ablinker`）

`abpc` は **自分と同じディレクトリ** にある `abc` / `abassembler` / `ablinker` を呼ぶ。  
...
```
# stage0 → stage1
bin\stage0\abpc.exe abc.pj -o bin\stage1\abc.exe
bin\stage0\abpc.exe abassembler.pj -o bin\stage1\abassembler.exe
bin\stage0\abpc.exe ablinker.pj -o bin\stage1\ablinker.exe
bin\stage0\abpc.exe abpc.pj -o bin\stage1\abpc.exe

# stage1 → stage2（自立確認）
bin\stage1\abpc.exe ... -o bin\stage2\...
```
