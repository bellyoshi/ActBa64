# run_test2.ps1 - test/ 全ソースを actba32 でコンパイル & 実行
# 使い方:
#   .\run_test2.ps1                      # bin\stage0\actba32.exe（既定）
...
#   .\run_test2.ps1 -Driver .\bin\stage2\actba32.exe
...
function Invoke-StageTests {
    ...
    if (-not (Test-Path -LiteralPath $DriverPath)) {
        Write-Error "actba32 not found: $DriverPath"
        return 2
    }
    ...
    Get-ChildItem -LiteralPath $TestDir -Filter "*.abp" -File | Sort-Object Name | ForEach-Object {
        ...
        & $DriverPath $relIn -o $exeName 2>&1 | Out-Null
...
# -Driver / -Abpc 指定時は単発（Stage は無視）
if ($Driver -ne "") {
    exit (Invoke-StageTests -DriverPath $Driver -Label $Driver)
}
...
$overall = 0
foreach ($s in $stages) {
    $path = Join-Path $Root "bin\$s\actba32.exe"
    $code = Invoke-StageTests -DriverPath $path -Label $s
